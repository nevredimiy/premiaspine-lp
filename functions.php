...
function premiaspine_enqueue_landing_codi_assets() {
	...
	// выбрать .min, если он существует и не старше исходника
	$pick = function ( $path ) {
		$min = preg_replace( '/\.(css|js)$/', '.min.$1', $path );
		return ( file_exists( $min ) && filemtime( $min ) >= filemtime( $path ) ) ? $min : $path;
	};
	$css_use = $pick( $css_file );
	$js_use  = $pick( $js_file );
	...
	if ( file_exists( $css_file ) ) {
		wp_enqueue_style(
			'premiaspine-landing',
			$theme_uri . '/css/' . basename( $css_use ),
...
function premiaspine_landing_print_wpgmp_runtime_assets( $inline_js = '' ) {
	...
	(function () {
		...
		// Safety net: first real interaction anywhere on the page. No idle/timeout
		// fallback — the map is well below the fold, so a visitor who never scrolls
		// near it and never interacts simply never needs it, and keeping the Maps
		// API's long tasks off such loads is the whole point.
