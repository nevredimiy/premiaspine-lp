<?php
/*
 *  Template Name: Lading page 05.2026
 */
?>
<?php get_header('landing_052026'); ?>
...
$top_section_bg = !empty($options['header']['top_section_bg']) ? wp_get_attachment_image_url($options['header']['top_section_bg'], 'full') : '';
...
<section class="top-section">
    <div class="holder"<?php if ( $top_section_bg ) : ?> style="background-image: url('<?php echo esc_url($top_section_bg); ?>');"<?php endif; ?>>
        <h1><?php echo $options['header']['top_title'] ?></h1>
        <div class="top-section-content">
            <?php echo apply_filters('the_content',$options['header']['top_content']); ?>
        </div>
        <div class="person">
            <span class="name"><?php echo $options['header']['doctror_name'] ?></span>
            <?php if ( ! empty( $options['header']['doctor_photo'] ) ) : ?>
                <img src="<?php echo esc_url( wp_get_attachment_image_url( $options['header']['doctor_photo'], 'full' ) ); ?>" alt="">
            <?php else : ?>
                <img src="<?php echo esc_url( get_stylesheet_directory_uri() . '/images/person.png' ); ?>" alt="Person">
            <?php endif; ?>
        </div>
...
<div id="main" class="main-alternate">
    <?php if ( !empty( $options['intro_content']['title']) || !empty( $options['intro_content']['advantages']) ) : ?>
        <section class="intro-content">
            <div class="container">
                <?php if ( !empty( $options['intro_content']['title']) ) : ?>
                    <h2><?php echo $options['intro_content']['title']?></h2>
                <?php endif; ?>
                <?php if ( !empty( $options['intro_content']['advantages']) ) : ?>
                    <ul class="advantages">
    ...
    <?php endif; ?>
    <?php if ( !empty($options['header']['title_benefits']) || !empty($options['header']['benefits'] ) ) : ?>
        <div class="benefits-section">
            <div class="benefits-list">
                <?php if ( !empty($options['header']['title_benefits']) ) : ?>
                    <h3><?php echo $options['header']['title_benefits']?></h3>
                <?php endif; ?>
                <?php if ( !empty($options['header']['benefits'] ) ) : ?>
                    <ul>
    ...
    <?php endif; ?>
    <?php if ( !empty($options['request_an_appointment_form_1']['request_an_appointment_form']) ) : ?>
    ...
    <?php endif; ?>
    <?php if ( !empty($options['content_area']['title']) || !empty($options['content_area']['text']) ) : ?>
        <section class="content-area area-3">
            <div class="container">
                <?php if ( !empty($options['content_area']['title']) ) : ?>
                    <h2><?php echo $options['content_area']['title']?></h2>
                <?php endif; ?>
                <?php if ( !empty($options['content_area']['text']) ) : ?>
    ...
    <?php endif; ?>
    <?php if ( empty($options['testimonials']['hide']) ) : ?>
    ...
    <?php endif; ?>
    <?php if ( !empty($options['request_an_appointment_form_2']['request_an_appointment_form']) ) : ?>
        <div class="request-appointment gray">
            <?php echo do_shortcode($options['request_an_appointment_form_2']['request_an_appointment_form']); ?>
        </div>
    <?php endif; ?>
    <?php if ( $options['questions_and_answer']['hide'] != 'disable' ) : ?>
    ...
    <section class="locations-section">
        <div class="container">
            ...
            <div class="into-area">
                ...
                <div class="map">
                    <?php
                    if(!empty($options['locations']['map_shortcode'])){
                        echo do_shortcode($options['locations']['map_shortcode']);
                    }
                    else{
